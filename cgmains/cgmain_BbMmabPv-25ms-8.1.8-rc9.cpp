...
vector<string> MinMaxAlphaBetaAgent::get_stats()
{
    ...
    stats.push_back("version=BbMmabPv-25ms-8.1.8-rc9");
    stats.push_back("depth=" + to_string(this->_depth_reached));
    stats.push_back("states=" + to_string(this->_nodes_explored));
    cerr << "BbMmabPv-25ms-8.1.8-rc9\t: stats=" << stats[0] << " " << stats[1] << " " << stats[2]
         << endl;
    return stats;
}
...
float MinMaxAlphaBetaAgent::minmax(Board *board, int max_depth, int depth, float alpha, float beta)
{
    ...
    if (depth == max_depth || this->is_time_up() || board->get_game_state(true) != GAME_CONTINUE)
