...
Move::Move(int _src_x, int _src_y, int _dst_x, int _dst_y, char _promotion)
{
    ...
    this->dst_x = _dst_x;
    this->dst_y = _dst_y;
    this->promotion = tolower(_promotion); // Promotion is always lowercase, because
                                           // its UCI representation is lowercase
}
...
string Move::to_uci(bool regular_rules_castling)
{
    ...
    // Normal move + castles:   e2e4
    string uci =
        coord_to_algebraic(this->src_x, this->src_y) + coord_to_algebraic(tmp_x, this->dst_y);
...
bool Move::operator==(Move *other)
{
    return this->src_x == other->src_x && this->src_y == other->src_y &&
           this->dst_x == other->dst_x && this->dst_y == other->dst_y &&
