...
int apply_move_testLauncher()
{
    ...
    // 2 "en passant" in a row
    successCount += apply_move_validMove_ApplyIt(
        22, new Board("8/1p6/8/2P5/4P3/8/8/8 b - e4 0 1"),
        new Board("8/8/8/1pP5/4P3/8/8/8 w - b6 0 2"), new Move(1, 1, 1, 3, 0)
    );

    // Regular White moves - (Don't reset the half turn rule)
    successCount += apply_move_validMove_ApplyIt(
        3, new Board("8/8/8/8/8/8/8/3Q4 w - - 5 1"), new Board("8/8/8/8/6Q1/8/8/8 b - - 6 1"),
        new Move(3, 7, 6, 4, 0)
    );
    // 2 advance white pawn that create a "en passant"
    successCount += apply_move_validMove_ApplyIt(
        4, new Board("8/8/8/8/3p4/8/4P3/8 w - - 0 1"), new Board("8/8/8/8/3pP3/8/8/8 b - e3 0 1"),
        new Move(4, 6, 4, 4, 0)
    );

    // Black Chess960 Castle left
    successCount += apply_move_validMove_ApplyIt(
        5, new Board("r3k3/8/8/8/8/8/8/8 b a - 0 1"), new Board("2kr4/8/8/8/8/8/8/8 w - - 1 2"),
        new Move(4, 0, 0, 0, 0)
    );
    // Black Chess960 Castle right
    successCount += apply_move_validMove_ApplyIt(
        6, new Board("1k5r/8/8/8/8/8/8/8 b h - 0 1"), new Board("5rk1/8/8/8/8/8/8/8 w - - 1 2"),
        new Move(1, 0, 7, 0, 0)
    );
    // White Chess960 Castle left
    successCount += apply_move_validMove_ApplyIt(
        7, new Board("8/8/8/8/8/8/8/R5K1 w A - 0 1"), new Board("8/8/8/8/8/8/8/2KR4 b - - 1 1"),
        new Move(6, 7, 0, 7, 0)
    );
    // White Chess960 Castle right
    successCount += apply_move_validMove_ApplyIt(
        8, new Board("8/8/8/8/8/8/8/4K2R w H - 0 1"), new Board("8/8/8/8/8/8/8/5RK1 b - - 1 1"),
        new Move(4, 7, 7, 7, 0)
    );
    // White king takes black rook, not castling
    successCount += apply_move_validMove_ApplyIt(
        88, new Board("8/8/1r6/K7/8/8/8/8 w - - 2 78"), new Board("8/8/1K6/8/8/8/8/8 b - - 0 78"),
        new Move(0, 3, 1, 2, 0)
    );

    // Black Standard Castle left
    successCount += apply_move_validMove_ApplyIt(
        51, new Board("r3k3/8/8/8/8/8/8/8 b a - 0 1", false),
        new Board("2kr4/8/8/8/8/8/8/8 w - - 1 2", false), new Move(4, 0, 0, 0, 0)
    );
    // Black Standard Castle right
    successCount += apply_move_validMove_ApplyIt(
        61, new Board("1k5r/8/8/8/8/8/8/8 b h - 0 1", false),
        new Board("5rk1/8/8/8/8/8/8/8 w - - 1 2", false), new Move(1, 0, 7, 0, 0)
    );
    // White Standard Castle left
    successCount += apply_move_validMove_ApplyIt(
        71, new Board("8/8/8/8/8/8/8/R5K1 w A - 0 1", false),
        new Board("8/8/8/8/8/8/8/2KR4 b - - 1 1", false), new Move(6, 7, 0, 7, 0)
    );
    // White Standard Castle right
    successCount += apply_move_validMove_ApplyIt(
        81, new Board("8/8/8/8/8/8/8/4K2R w H - 0 1", false),
        new Board("8/8/8/8/8/8/8/5RK1 b - - 1 1", false), new Move(4, 7, 7, 7, 0)
    );

    // Castles rights deletion
    successCount += apply_move_validMove_ApplyIt(
        82, new Board("8/8/8/8/8/8/8/R3K2R w AH - 0 1"), new Board("8/8/8/8/8/8/4K3/R6R b - - 1 1"),
        new Move(4, 7, 4, 6, 0)
    );
    successCount += apply_move_validMove_ApplyIt(
        83, new Board("8/8/8/8/8/8/8/R3K2R w AH - 0 1"), new Board("8/8/8/8/8/8/R7/4K2R b H - 1 1"),
        new Move(0, 7, 0, 6, 0)
    );
    successCount += apply_move_validMove_ApplyIt(
        84, new Board("8/8/8/8/8/8/8/R3K2R w AH - 0 1"),
        new Board("8/8/8/8/8/8/7R/R3K21 b A - 1 1"), new Move(7, 7, 7, 6, 0)
    );

    // Promotion White
    successCount += apply_move_validMove_ApplyIt(
        9, new Board("8/5P2/8/8/8/8/8/8 w - - 0 1"), new Board("5Q2/8/8/8/8/8/8/8 b - - 0 1"),
        new Move(5, 1, 5, 0, 'Q')
    );
    successCount += apply_move_validMove_ApplyIt(
        10, new Board("8/3P4/8/8/8/8/8/8 w - - 0 1"), new Board("3N4/8/8/8/8/8/8/8 b - - 0 1"),
        new Move("d7d8n")
    );

    // Promotion Black
    successCount += apply_move_validMove_ApplyIt(
        11, new Board("8/8/8/8/8/8/5p2/8 b - - 0 1"), new Board("8/8/8/8/8/8/8/5r2 w - - 0 2"),
        new Move(5, 6, 5, 7, 'r')
    );
    successCount += apply_move_validMove_ApplyIt(
        12, new Board("8/8/8/8/8/8/3p4/8 b - - 0 1"), new Board("8/8/8/8/8/8/8/3b4 w - - 0 2"),
        new Move("d2d1b")
    );

    // En passant White
    successCount += apply_move_validMove_ApplyIt(
        13, new Board("8/8/8/4pP2/8/8/8/8 w - b3 0 1"), new Board("8/8/4P3/8/8/8/8/8 b - - 0 1"),
        new Move(5, 3, 4, 2, 0)
    );
    successCount += apply_move_validMove_ApplyIt(
        14, new Board("8/8/8/8/2pP4/8/8/8 w - d3 0 1"), new Board("8/8/8/8/8/3p4/8/8 b - - 0 1"),
        new Move(2, 4, 3, 5, 0)
    );

    // Reset the half turn rule when a piece is captured
    successCount += apply_move_validMove_ApplyIt(
        15, new Board("8/8/4r3/8/8/8/4B3/8 b - - 5 1"), new Board("8/8/8/8/8/8/4r3/8 w - - 0 2"),
        new Move(4, 2, 4, 6, 0)
    );
...
int find_pawn_moves_testLauncher()
{
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        0, new Board("8/8/8/8/6#1/5#2/5PPP/8 w - - 0 1"), requested_moves, 3
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        1, new Board("8/ppp5/#7/1#6/8/8/8/8 b - - 0 1"), requested_moves, 3
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        2, new Board("8/8/2T#T3/3P4/8/#t4t#/P6P/1t4t1 w - - 0 1"), requested_moves, 2
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        3, new Board("1T4T1/p6p/#T4T#/8/3p4/2t#t3/8/8 b - - 0 1"), requested_moves, 2
    );

    // "en passant" - White - Left
    requested_moves[0] = new Move(0, 3, 1, 2, 0);
    success_count += find_moves_RegularCases_FindAllMoves(
        4, new Board("8/8/#7/Pp6/1#6/8/8/8 w - b6 0 1"), requested_moves, 1
    );
    // "en passant" - White - Right
    requested_moves[0] = new Move(2, 3, 1, 2, 0);
    success_count += find_moves_RegularCases_FindAllMoves(
        5, new Board("8/8/2#5/1pP5/1#6/8/8/8 w - b6 0 1"), requested_moves, 1
    );

    // "en passant" - Black - Left
    requested_moves[0] = new Move(1, 4, 0, 5, 0);
    success_count += find_moves_RegularCases_FindAllMoves(
        6, new Board("8/8/8/#7/Pp6/1#6/8/8 b - a3 0 1"), requested_moves, 1
    );
    // "en passant" - Black - Right
    requested_moves[0] = new Move(1, 4, 2, 5, 0);
    success_count += find_moves_RegularCases_FindAllMoves(
        7, new Board("8/8/8/2#5/1pP5/1#6/8/8 b - c3 0 1"), requested_moves, 1
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        8, new Board("7#/P6P/8/8/8/8/8/8 w - - 0 1"), requested_moves, 4
    );
    // Promotions - Black
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        9, new Board("8/8/8/8/8/8/p6p/7# b - - 0 1"), requested_moves, 4
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        10, new Board("t1t5/1P6/8/8/8/8/8/8 w - - 0 1"), requested_moves, 12
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        11, new Board("8/8/8/8/8/8/3p4/2T1T3 b - - 0 1"), requested_moves, 12
    );

    // No piece -> No moves
    success_count += find_moves_RegularCases_FindAllMoves(
        12, new Board("8/8/8/8/8/8/8/8 w - - 0 1"), requested_moves, 0
    );

    // Find "en_passant" move to remove check
    requested_moves[0] = new Move(2, 3, 3, 2, 0);
    success_count += find_moves_RegularCases_FindAllMoves(
        13, new Board("8/8/2#5/1#Pp4/2K5/1#6/3q4/8 w - d6 0 30"), requested_moves, 1
    );
...
int find_knight_moves_testLauncher()
{
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        11, new Board("8/8/2N5/8/8/8/8/8 w - - 0 1"), requested_moves, 8
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        12, new Board("8/8/8/8/8/5n2/8/8 b - - 0 1"), requested_moves, 8
    );
    ...
    requested_moves[7] = new Move(2, 2, 3, 4, 0); // 2 down 1 right
    success_count += find_moves_RegularCases_FindAllMoves(
        13, new Board("1p1p4/p3p3/2N5/p3p3/1p1p4/8/8/8 w - - 0 1"), requested_moves, 8
    );
    ...
    requested_moves[7] = new Move(5, 5, 6, 7, 0); // 2 down 1 right
    success_count += find_moves_RegularCases_FindAllMoves(
        14, new Board("8/8/8/1P1P4/P3P3/5n2/P3P3/1P1P4 b - - 0 1"), requested_moves, 8
    );

    // No moves - Left up corner - White
    success_count += find_moves_RegularCases_FindAllMoves(
        15, new Board("N1#5/1#P5/1P6/8/8/8/8/8 w - - 0 1"), requested_moves, 0
    );
    // No moves - Left up corner - Black
    success_count += find_moves_RegularCases_FindAllMoves(
        16, new Board("n7/2p5/1p#5/1#6/8/8/8/8 b - - 0 1"), requested_moves, 0
    );

    // No moves - Right up corner - White
    success_count += find_moves_RegularCases_FindAllMoves(
        17, new Board("5#1N/5P#1/6P1/8/8/8/8/8 w - - 0 1"), requested_moves, 0
    );
    // No moves - Right up corner - Black
    success_count += find_moves_RegularCases_FindAllMoves(
        18, new Board("7n/5p2/5#p1/6#1/8/8/8/8 b - - 0 1"), requested_moves, 0
    );

    // No moves - Left down corner - White
    success_count += find_moves_RegularCases_FindAllMoves(
        19, new Board("8/8/8/8/1#6/1P#5/2P5/N7 w - - 0 1"), requested_moves, 0
    );
    // No moves - Left down corner - Black
    success_count += find_moves_RegularCases_FindAllMoves(
        20, new Board("8/8/8/8/8/1p6/1#p5/n1#5 b - - 0 1"), requested_moves, 0
    );

    // No moves - Right down corner - White
    success_count += find_moves_RegularCases_FindAllMoves(
        21, new Board("8/8/8/8/6#1/5#P1/5P2/7N w - - 0 1"), requested_moves, 0
    );
    // No moves - Right down corner - Black
    success_count += find_moves_RegularCases_FindAllMoves(
        22, new Board("8/8/8/8/8/6p1/5p#1/5#1n b - - 0 1"), requested_moves, 0
    );
...
int find_bishop_moves_testLauncher()
{
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        23, new Board("6T1/8/8/3B4/2T5/5T2/8/8 w - - 0 1"), requested_moves, 6
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        24, new Board("7t/8/8/4b3/3t4/6t1/8/8 b - - 0 1"), requested_moves, 6
    );

    // Captures - White
    requested_moves[0] = new Move(0, 7, 1, 6, 0); // Right up
    success_count += find_moves_RegularCases_FindAllMoves(
        25, new Board("8/8/8/8/8/8/1t6/B7 w - - 0 1"), requested_moves, 1
    );

    // Captures - Black
    requested_moves[0] = new Move(7, 0, 6, 1, 0); // Left down
    success_count += find_moves_RegularCases_FindAllMoves(
        26, new Board("7b/6T1/8/8/8/8/8/8 b - - 0 1"), requested_moves, 1
    );
...
int find_rook_moves_testLauncher()
{
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        27, new Board("8/8/8/3R4/8/8/8/8 w - - 0 1"), requested_moves, 14
    );
    success_count += find_moves_RegularCases_FindAllMoves(
        28, new Board("8/8/8/3r4/8/8/8/8 b - - 0 1"), requested_moves, 14
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        29, new Board("8/8/3t4/2tRt3/3t4/8/8/8 w - - 0 1"), requested_moves, 4
    );
    success_count += find_moves_RegularCases_FindAllMoves(
        30, new Board("8/8/3T4/2TrT3/3T4/8/8/8 b - - 0 1"), requested_moves, 4
    );

    // Blocked
    success_count += find_moves_RegularCases_FindAllMoves(
        31, new Board("8/8/3T4/2TRT3/3T4/8/8/8 w - - 0 1"), requested_moves, 0
    );
    success_count += find_moves_RegularCases_FindAllMoves(
        32, new Board("8/8/3t4/2trt3/3t4/8/8/8 b - - 0 1"), requested_moves, 0
    );
...
int find_king_moves_testLauncher()
{
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        33, new Board("8/8/8/3K4/8/8/8/8 w - - 0 1"), requested_moves, 8
    );
    success_count += find_moves_RegularCases_FindAllMoves(
        34, new Board("8/8/8/3k4/8/8/8/8 b - - 0 1"), requested_moves, 8
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        35, new Board("8/8/8/8/8/8/###4#/RK5R w AH - 0 1"), requested_moves, 8
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        36, new Board("1#r1k1r#/########/8/8/8/8/8/8 b cg - 0 1"), requested_moves, 6
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        37, new Board("8/8/8/8/4r3/8/####1###/##K3R# w G - 0 1"), requested_moves, 4
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        38, new Board("8/8/8/8/4r3/8/###5/#KR4# w C - 0 1"), requested_moves, 4
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        39, new Board("##3rk#/5###/8/8/8/3R4/8/8 b f - 0 1"), requested_moves, 3
    );

    // No castles - King under check
    requested_moves[0] = new Move(3, 7, 4, 7, 0); // King moves right
    success_count += find_moves_RegularCases_FindAllMoves(
        40, new Board("8/8/8/8/8/3r4/2#1####/2#K2R# w G - 0 1"), requested_moves, 1
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        41, new Board("8/8/8/8/8/8/2#####r/3#K2R w H - 0 1"), requested_moves, 5
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        42, new Board("8/8/8/8/8/8/5###/2#2RK1 w F - 0 1"), requested_moves, 3
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        43, new Board("8/8/8/8/8/8/###5/1KR3#1 w F - 0 1"), requested_moves, 4
    );
    ...
    requested_moves[3] = new Move(2, 7, 6, 7, 0); // Rook moves left 4
    success_count += find_moves_RegularCases_FindAllMoves(
        44, new Board("8/8/8/8/6r1/8/###5/#KR4# w C - 0 1"), requested_moves, 4
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        45, new Board("8/8/8/8/8/8/###5/RKR2#2 w A - 0 1"), requested_moves, 2
    );

    // No castles - Blocked
    success_count += find_moves_RegularCases_FindAllMoves(
        46, new Board("bbqrkrnn/pppppppp/########/8/P7/8/1PPPPPPP/BBQRKRNN b DFdf a3 0 1"),
        requested_moves, 0
    );

    // No castles - Extremety 1
    success_count += find_moves_RegularCases_FindAllMoves(
        47, new Board("8/8/8/8/8/8/###5/RK#T4 w A - 1 6"), requested_moves, 0
    );

    // No castles - Extremety 2
    success_count += find_moves_RegularCases_FindAllMoves(
        48, new Board("8/8/8/8/8/8/5###/5TKR w H - 1 6"), requested_moves, 0
    );

    // No castles - Extremety 2
    success_count += find_moves_RegularCases_FindAllMoves(
        49, new Board("8/8/8/8/8/8/########/4BRKR w FH - 3 11"), requested_moves, 0
    );
...
int find_moves_not_illegal_ones_testLauncher()
{
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        50, new Board("8/1R6/2B5/7r/K6r/4P3/6Q1/8 w - - 0 1"), requested_moves, 7
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        51, new Board("3q4/2p5/3R3r/8/2#1#3/2#k#1b1/2###3/8 b - - 0 1"), requested_moves, 4
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        52, new Board("8/8/4q3/4Q3/2qRKPq1/4BN2/4q1b1/8 w - - 0 1"), requested_moves, 2
    );
    ...
    success_count += find_moves_RegularCases_FindAllMoves(
        53, new Board("8/8/8/KPp4r/8/8/8/8/8 w - c6 0 1"), requested_moves, 4
    );
